...
function renderHeader($title, $desc, $pageName, $headerRaw) {
...
function renderFooter($footerRaw) {
...
foreach ($pages as $phpFile => $meta) {
    ...
    $fullHtml = renderHeader($title, $desc, $htmlFile, $headerRaw) . "\n" . $content . "\n" . renderFooter($footerRaw);

    file_put_contents($baseDir . '/' . $htmlFile, $fullHtml);
    echo "Generated " . $htmlFile . "\n";
}

echo "Vercel export complete!";
?>
